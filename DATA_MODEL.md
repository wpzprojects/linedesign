...
Opcional; ausente hasta que el usuario presiona el botón "Ajustar al terreno real" en la cabecera de Perfil. Array de `{ station, elevation }` — un muestreo del alineamiento **cada metro** (paso más grueso solo si el trazado supera los 10 km, para no pedir un lote absurdamente grande de una sola vez) de la elevación real, consultada a un servicio de elevación (`src/data/elevationSource.js`, Open-Elevation) vía `geo.epsg9377ToLatLon` para convertir cada punto muestreado a lat/lon. Cuando está presente, `profileView.js` dibuja este perfil denso en vez de la interpolación lineal entre vértices (más fiel al terreno real, incl. picos/valles entre PIs que la sola `vertex.z` no captura) — ver `.profile-line--real` en `styles.css`.
