...
- **Barra de actividad** (extremo izquierdo, ~56px, nunca se esconde): íconos para cambiar de pantalla (Parámetros de entrada / Planta y Perfil / Catálogo / Cálculo mecánico / Resumen), más el engranaje de Configuración y el signo de pregunta de Ayuda al fondo. El toggle de tema claro/oscuro vive en la barra de título (esquina superior derecha), no en la barra de actividad.
- **Ayuda**: documentación de toda la aplicación (qué hace cada pantalla, cada control, y las simplificaciones de Fase 1/2 frente a un software de diseño real), en la propia app.
- **Configuración**: ajustes de la app, no del proyecto — persisten en localStorage, no viajan en el JSON exportado. Por ahora, colores del lienzo: color de conductores (Perfil) y de postes (Planta y Perfil), con botón para restablecer el color por defecto de cada tema.
...
- **Resumen**: tarjeta Resumen del proyecto, Tabla de estructuras (estación, altura, cotas, vano/flecha/distancia mínima al piso del vano adelante bajo la hipótesis de referencia) y Alineamiento (estación, Este/Norte/elevación de cada vértice) — clic en una fila selecciona y salta a Planta y Perfil.
