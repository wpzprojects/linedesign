...
`catenary.resolveReferenceTension(conductor, referenceHypothesis, spanLength, stringingTensions)` busca filas donde `weatherCase` coincida con el nombre de la hipótesis de referencia (`conductor.referenceHypothesisId`, NO el selector "Catenaria bajo hipótesis" de Planta y Perfil, que solo cambia qué hipótesis se dibuja) y `applicableCable` esté vacío o coincida con el conductor. Por cada fila que aplique calcula `H = min(%rotura·RTS, tensión máxima, peso_vertical·catenaria_máxima)` (solo los límites presentes); si aplican varias filas, se usa la más restrictiva (mínima). Sin ninguna fila que aplique, cae a `conductor.referenceHorizontalTension` (el campo manual) y `app.js` avisa una vez por combinación conductor+hipótesis rota, solo si la tabla tiene al menos una fila cargada (si está vacía del todo, es "no se usa este criterio", no una configuración incompleta).
