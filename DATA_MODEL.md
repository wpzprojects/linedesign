...
Opcional; ausente hasta que el usuario presiona el botón "Ajustar al terreno real" en la cabecera de Perfil. Array de `{ station, elevation }` — un muestreo del alineamiento cada 25 m (paso más grueso si el trazado supera unos 12.5 km, para no exceder ~500 puntos en una sola operación) de la elevación real, consultada a un servicio de elevación (`src/data/elevationSource.js`) vía `geo.epsg9377ToLatLon` para convertir cada punto muestreado a lat/lon. **Por qué 25 m y no menos**: los datasets tipo SRTM que sirven estos servicios gratuitos tienen resolución real de ~30 m — muestrear más seguido que eso no agrega detalle de terreno. Cuando el perfil está presente, `profileView.js` lo dibuja en vez de la interpolación lineal entre vértices (más fiel al terreno real, incl. picos/valles entre PIs que la sola `vertex.z` no captura) — ver `.profile-line--real` en `styles.css`.
...
Ninguno de los dos es perfecto — son servicios gratuitos de terceros ("pueden ser lentos o inestables bajo carga", prompt maestro Apéndice A.2) — así que el fallback automático entre ambos es más robusto que apostarlo todo a uno solo.

**Si eso no alcanza** (terreno muy accidentado, se sigue viendo "en escalones"), la pantalla "Configuración" permite cambiar el proveedor a la **Google Elevation API** con una key propia del usuario — `fetchElevations(points, { provider, apiKey })` en `elevationSource.js`. La preferencia (`provider`, `apiKey`) es un ajuste de **la app**, no del proyecto: se guarda en `localStorage` (`linedesign-elevation-provider`/`linedesign-elevation-google-key`), no en el JSON exportado. La key queda visible en las herramientas de desarrollador del navegador, como cualquier key usada desde una app 100% de cliente (mismo caveat que ya documentaba el prompt maestro sobre Mapbox/MapTiler) — Google además exige una cuenta de facturación activa en el proyecto para habilitar la Elevation API, aunque da crédito mensual gratis.
