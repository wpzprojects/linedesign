...
- **Capacidad**: como un poste de sección circular no tiene eje débil/fuerte, la carga de ensayo actúa a `(structureAboveGroundHeight − 0.20 m)` de la base (línea de terreno) — `structureAboveGroundHeight` es la altura LIBRE real (descuenta la profundidad de empotramiento si el tipo la tiene activada, ver `loadTree.js#structureAboveGroundHeight`; si no, es `structure.height` tal cual). De ahí el momento último en la base: `M_última = resistance × (structureAboveGroundHeight − 0.20)`. Se divide entre `project.poleSafetyFactor` (configurable en Parámetros de entrada § Terreno, por defecto 2) para obtener el momento admisible.
- **Demanda SIN contraviento**: la resultante de las fuerzas horizontales del árbol de cargas, `√(transversal² + longitudinal²)`, aplicada a la altura promedio de enganche (`attachHeight`, la misma que usa `momentEstimate` — también sobre `structureAboveGroundHeight`), evaluada para TODAS las hipótesis climáticas — se toma la peor (mayor momento), no solo la de referencia.
...
Habilitable en Propiedades solo para estructuras que anclan la línea (Retención/Ángulo — Suspensión/Paso quedan deshabilitadas, el conductor las atraviesa sin generar desequilibrio de tensión que anclar). Campos nuevos por estructura: `guyResistance` (kgF, elegido de `type.guyResistanceOptions` — resistencia ÚLTIMA del cable de contraviento), `guyAnchorHeight` (m, distancia desde la PUNTA del poste hacia abajo hasta el punto de enganche — mismo criterio que `attachmentPoints[].offsetZ`; hoy es solo geometría de referencia, sugerida automáticamente, no entra en ningún cálculo — ver limitación conocida más abajo) y `guyAnchorAngle` (°, ángulo del cable respecto a la VERTICAL del poste — sugerido 30°, este SÍ define la tracción del cable).
...
**LIMITACIÓN CONOCIDA (Fase 1, pendiente — ver IDEAS_FUTURAS.md)**: al no usar `guyAnchorHeight` en el cálculo, el modelo asume implícitamente que el contraviento está enganchado a la misma altura que el conductor. Si en la instalación real el contraviento queda enganchado más abajo que el punto de amarre del conductor, el segmento de poste entre ambos puntos sigue sometido a un momento flector no cancelado por el contraviento, que hoy no se verifica.

**Tracción en el cable de contraviento**: la fuerza horizontal que el contraviento resiste (la misma que se le resta al poste) se convierte a tracción real del cable con la geometría del anclaje: `T_cable = F_horizontal_resistida / sen(θ)`, con `θ = guyAnchorAngle` (ángulo respecto a la VERTICAL del poste — mientras más cerrado/vertical, mayor la tracción necesaria para la misma carga). Se compara contra `structure.guyResistance / project.guySafetyFactor` (factor de seguridad propio, independiente del de postes).
...
Pertenecen al **tipo de estructura** (catálogo), no a cada instancia — todas las estructuras de un mismo tipo comparten geometría de fijación. `offsetX` es el desplazamiento lateral (m) respecto al eje de la estructura; `offsetZ` es la distancia desde la PUNTA del poste hacia abajo hasta el punto de enganche (m; no desde el piso — así el offset sigue siendo válido sin importar qué `heightOptions` tenga la estructura). Se usan para: (a) la elevación de enganche real de cada fase en Perfil/DXF/Resumen (ver debajo) y (b) el número de fases para el árbol de cargas (`loadTree.js` multiplica las fuerzas por vano por `attachmentPoints.length`).

`profileView.js`/`dxfExport.js` dibujan **una curva de catenaria por fase real** (emparejadas por posición entre las dos estructuras del vano, `Math.min(fromPoints.length, toPoints.length)`), cada una colgando de su propia altura (`structureAboveGroundHeight − offsetZ`) — no una sola curva promedio. La flecha y la distancia de seguridad al terreno que se muestran/exportan (etiquetas, Resumen § Tabla de estructuras) son las de la fase físicamente MÁS BAJA de las dos estructuras del vano, la que de verdad arriesga el "Cumple" de distancia de seguridad. El árbol de cargas (`loadTree.js`), aparte, pondera por el número de fases reales del tipo pero no por su altura individual (usa `averageAttachmentHeight`, el promedio). Los `offsetX` de cada fase no se usan aún para separar geometría lateral 3D — Fase 3 (vista 3D).
